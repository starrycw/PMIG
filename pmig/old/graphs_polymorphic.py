...
class Literal_Map:
    ...
    def add_A_mapping(self, literal_A, literal_new):
        ...
        l_A = PMIG.get_noattribute_literal(f=literal_A)
        l_new = PMIG.get_noattribute_literal(f=literal_new)
    ...
    def add_B_mapping(self, literal_B, literal_new):
        ...
        # l_new = PMIG.get_positive_normal_literal(literal_new)
        l_B = PMIG.get_noattribute_literal(literal_B)
    ...
    def active_pi_merge(self, pi_l, new_l, subgraph):
        ...
        :return:
        '''
        assert PMIG.is_noattribute_literal(new_l)
        assert PMIG.is_noattribute_literal(pi_l)
        if subgraph == 'A':
...
class PMIG_Generation_combinational:
    ...
    def __init__(self, mig1, mig2):
        ...
        self._mig_a = copy.deepcopy(mig1)  # The PMIG obj of function A
        self._mig_b = copy.deepcopy(mig2)  # The PMIG obj of function B
        assert not self.is_polymorphic_mig(self._mig_a), "[ERROR]graphs_polymorphic: The input PMIG (mig1) cannot be polymorphic!"
        assert not self.is_polymorphic_mig(self._mig_b), "[ERROR]graphs_polymorphic: The input PMIG (mig2) cannot be polymorphic!"
        assert self.is_combinational_mig(self._mig_a)
        assert self.is_combinational_mig(self._mig_b)
    ...
    def mux_fanin_list_add(self, t):
        ...
        for i in t:
            ...
            assert len(i) == 2
            assert i not in self._mux_fanins
    ...
    def merged_pis_list_add(self, t):
        ...
        for i in t:
            assert isinstance(i, tuple)
            assert len(i) == 2
            assert i not in self._merged_pis
